...
namespace MAVN.Service.NotificationSystemAudit.Tests
{
    public class ClientHttpPathsAreMatchedInControllersTest
    {
        ...
        {
            var clientInterface = typeof(INotificationSystemAuditClient);

            var apiInterfaces = clientInterface
                .GetProperties()
                .Where(p => p.CanRead && p.PropertyType.IsInterface)
                .Select(p => p.PropertyType)
                .Where(x => x.GetMethods().Any(y => y.Name != "IsAlive"))
                .ToList();
